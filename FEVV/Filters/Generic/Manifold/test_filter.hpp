...
void
test_filter(const HalfedgeGraph &g,
                   PointMap &pm,
                   /*VertexColorMap &v_cm,
...
//  }
  ...
  auto halfedge_iterator_pair = halfedges(g);
  halfedge_iterator hi = halfedge_iterator_pair.first;
  halfedge_iterator hi_end = halfedge_iterator_pair.second;
 // // recuperation des tailles
 // std::map < *halfedge_iterator, int> hmap;

 // for (; hi != hi_end; ++hi)
 // {
	//  hmap.insert(std::pair<edge_descriptor, int>(hi, 0 /*taille*/));
 // }
 // 
 // // 
	//std::cout << "piou";
...
// Helper function to simplify (syntactic sugar) the call to test_filter
template< typename HalfedgeGraph,
          typename PointMap,
          /*typename VertexColorMap,
          typename VertexNormalMap,*/
